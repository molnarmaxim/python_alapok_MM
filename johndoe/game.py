    ...
    def save_game():
        with open('savegame.pkl', 'wb') as f:
            pickle.dump({
                ...
                'businesses': [(b.name, b.base_cost, b.base_income, b.level) for b in businesses],  # Save business details
                'achievements': achievements_reached
            }, f)

    def load_game():
        global money, prestige_points, income_multiplier, businesses, achievements_reached
        try:
            with open('savegame.pkl', 'rb') as f:
                ...
                # Recreate business instances
                businesses = [Business(name, base_cost, base_income, level) for name, base_cost, base_income, level in data['businesses']]
                achievements_reached = data['achievements']
        except FileNotFoundError:
            print("No save file found.")

    # Bind save and load to buttons
    save_button = draw_button("Save", SCREEN_WIDTH - 150, SCREEN_HEIGHT - 150, 120, 40)
    load_button = draw_button("Load", SCREEN_WIDTH - 150, SCREEN_HEIGHT - 200, 120, 40)

    if mouse_clicked and save_button.collidepoint(mouse_pos):
        save_game()
    if mouse_clicked and load_button.collidepoint(mouse_pos):
        load_game()
